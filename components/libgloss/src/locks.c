...
static void lock_init_generic(xSemaphoreHandle *lock, uint8_t mutex_type) {
    portENTER_CRITICAL();
...
/* Free the mutex semaphore pointed to by *lock, and zero it out.
   ...
   this!
*/
static void lock_close_generic(xSemaphoreHandle *lock,  uint8_t mutex_type) {
    portENTER_CRITICAL();
    if (*lock) {
...
*/
static int lock_acquire_generic(xSemaphoreHandle *lock, uint32_t delay, uint8_t mutex_type) {
...
/* Release the mutex semaphore for lock.
   mutex_type is queueQUEUE_TYPE_RECURSIVE_MUTEX or queueQUEUE_TYPE_MUTEX
*/
static void lock_release_generic(xSemaphoreHandle *lock, uint8_t mutex_type) {
